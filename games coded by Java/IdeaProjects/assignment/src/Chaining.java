...
public class Chaining {
	...
	public int chain (int key) {
		int solution=((this.A * key ) % (power2(this.w))) >> (this.w- this.r);
		return solution;
	}
	...
	public int insertKey(int key){
		...
		int index = this.chain(key);
		if(this.Table.get(index).isEmpty()) {
			this.Table.get(index).set(0, key);
			return 0;
		}
		else {
			int element= this.Table.get(index).size();
			this.Table.get(index).set(element, key);
			return element;
		}	
